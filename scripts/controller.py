...
class Controller(Node):
    def __init__(self):
        ...
        # 4. Subscriber to the robot's odometry (current x, y position)
        self.create_subscription(Odometry, '/odom', self.process_odom, 10)

        # 5. Publisher of the robot's distance to the fixed point in the environment
        self.distancePublisher = self.create_publisher(Float32, '/distance', 10)

        # 6. Server to GetAvg service (average of the last 5 user inputs)
        self.avg_server = self.create_service(GetAvg, 'get_avg_service', self.get_avg_callback)

        # 7. Server to SetPoint service (change the fixed point used for the distance)
        self.set_point_server = self.create_service(SetPoint, 'set_point', self.set_point_callback)
        ...
        # Robot's current position (from /odom) and the fixed target point (assignment: 10, 10)
        self.xCurrent = 0.0
        self.yCurrent = 0.0
        self.xTarget = 10.0
        self.yTarget = 10.0
    ...
    def process_odom(self, msg):
        # Store the current position, compute the distance to the fixed point and publish it
        self.xCurrent = msg.pose.pose.position.x
        self.yCurrent = msg.pose.pose.position.y
        distance = math.hypot(self.xTarget - self.xCurrent, self.yTarget - self.yCurrent)
        self.distancePublisher.publish(Float32(data=distance))

    def set_point_callback(self, request, response):
        # Change the fixed point used for the distance computation
        self.xTarget = request.x
        self.yTarget = request.y
        response.success = True
        self.get_logger().info(f'Fixed point updated to: x={self.xTarget}, y={self.yTarget}')
        return response
